...
def ssd_net(inputs,
    ...
    with tf.variable_scope(scope, 'ssd_512_vgg', [inputs], reuse=reuse):
        ...
        with tf.variable_scope("block0"):
            net = slim.conv2d(inputs, 64, [5, 5], activation_fn=tf.nn.selu, stride=2,data_format=data_format)  # 256
        with tf.variable_scope("block1"):
            net=yolo(net,[32,64])
            net = yolo(net,[32, 64])
            print(net.shape)
        with tf.variable_scope("block2"):
            net = conv(net,128,(3,3),stride=2)#128
            net = yolo(net, [64, 128])
            net = yolo(net, [64, 128])
            print(net.shape)
        with tf.variable_scope("block3"):
            net = conv(net, 256, (3, 3), stride=2)  # 64
        ...
        with tf.variable_scope("block4"):
            net = conv(net, 256, (3, 3), stride=2)  # 32
            net = yolo(net, [128, 256])
            net = yolo(net, [128, 256])
            net = conv(net, 256, (3, 3))
            print(net.shape)
        ...
        with tf.variable_scope("block5"):
            net = conv(net, 256, (3, 3), stride=2)  # 16
            net = yolo(net, [128, 256])
            net = yolo(net, [128, 256])
            net = conv(net, 256, (3, 3))
            print(net.shape)
        ...
        with tf.variable_scope("block6"):
            net = conv(net, 256, (3, 3), stride=2)  # 8
            net = yolo(net, [128, 256])
            net = yolo(net, [128, 256])
            net = conv(net, 256, (3, 3))
            print(net.shape)
