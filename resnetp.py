...
class SSDNet(object):
    ...
    default_params = SSDParams(
        ...
        feat_layers=['block3','block4', 'block5','block6'],
        feat_shapes=[(64, 64),(32, 32), (16, 16),(8, 8)],
        anchor_size_bounds=[0.01, 0.90],
        anchor_sizes=[(24,42),
                      (56, 72),
                      (86, 104),
                      (128, 208)],
...
def ssd_anchor_one_layer(img_shape,
    ...
    # if feat_shape[0]==64:
    #     h=np.array([24,32,18,40,50,28],np.float32)
    #     w=np.array([24,18,32,40,28,50],np.float32)
    # elif feat_shape[0]==32:
    #     h=np.array([72,96,52,128,156,96],np.float32)
    #     w=np.array([72,52,96,128,96,156],np.float32)
    # elif feat_shape[0]==16:
    #     h=np.array([172,136,208],np.float32)
    #     w=np.array([172,208,138],np.float32)
    ...
    di = 1
    if len(sizes) > 1:
        h[1] = math.sqrt(sizes[1] * sizes[1]) / img_shape[0]
...
def unit_neck(input,fillter,scope,shortchut,stride=1):
    with tf.variable_scope("mob_unit" + scope):
            output = slim.conv2d(input, fillter[0], (1,1),activation_fn=None,data_format=data_format)
            ...
            output = slim.separable_conv2d(output, fillter[1], (3, 3), depth_multiplier=1,
                                           stride=stride,activation_fn=None,data_format=data_format)
            output = slim.batch_norm(output, activation_fn=None, is_training=training,data_format=data_format)
            output=tf.nn.relu(output)
            print('-----',scope, output.shape)
...
def conv(input,fillter,num,stride=1):
    output = slim.conv2d(input, num, fillter,activation_fn=None,data_format=data_format,stride=stride)
    output = slim.batch_norm(output, activation_fn=None, is_training=training,data_format=data_format)
    output=tf.nn.relu(output)
    return output

def modular(input,fillter,scope,shortchut,version=1,stride=1):
        output = slim.conv2d(input, fillter[0], (1,1),activation_fn=None,data_format=data_format)
        output = slim.batch_norm(output, activation_fn=None, is_training=training,data_format=data_format)
        output=tf.nn.relu(output)
        print('-----',scope, output.shape)
        output = slim.conv2d(output, fillter[1], (3,3),stride=stride,activation_fn=None,data_format=data_format)
        output = slim.batch_norm(output, activation_fn=None, is_training=training,data_format=data_format)
        output=tf.nn.relu(output)
        output = slim.conv2d(output, fillter[2], (1,1),activation_fn=None,data_format=data_format)
        output = slim.batch_norm(output, activation_fn=None, is_training=training,data_format=data_format)
        if shortchut==True:
            if version==2:
                ...
                output=tf.nn.selu(output+input)
        return output

def resnet(input,fillter,short=True,stride=1):
    preact = slim.batch_norm(input, activation_fn=None, is_training=training,data_format=data_format)
    output=tf.nn.leaky_relu(preact)
    output = slim.conv2d(output, fillter[0], (1,1),activation_fn=None,data_format=data_format)
    output = slim.batch_norm(output, activation_fn=None, is_training=training,data_format=data_format)
    output=tf.nn.leaky_relu(output)
    output = slim.conv2d(output, fillter[1], (3,3),activation_fn=None,data_format=data_format,stride=stride)
    output = slim.batch_norm(output, activation_fn=None, is_training=training,data_format=data_format)
    output=tf.nn.leaky_relu(output)
    output = slim.conv2d(output, fillter[2], (1,1),activation_fn=None,data_format=data_format)
    if short:
        if stride!=1:
            shortchut = slim.conv2d(preact, fillter[2], (3,3),activation_fn=None,data_format=data_format,stride=stride)
            shortchut=slim.batch_norm(shortchut, activation_fn=None, is_training=training,data_format=data_format)
            shortchut=tf.nn.leaky_relu(shortchut)
            output=output+shortchut
        else:
            output=output+preact
    return output
...
def ssd_net(inputs,
    ...
    with tf.variable_scope(scope, 'ssd_512_vgg', [inputs], reuse=reuse):
        print(inputs.shape)
        with tf.variable_scope("mobile_v2"):
            net=conv(inputs,[7,7],32,stride=2)  # 256
            print('block0',net.shape)
            with tf.variable_scope("block1"):
                net=resnet(net,[24, 24, 32])
                net=resnet(net,[24, 24, 32])
            print('block1',net.shape)
            with tf.variable_scope("block2"):#128
                net=resnet(net,[32, 32, 64],stride=2)
                net=resnet(net,[32, 32, 64])
            print('block2',net.shape)
            with tf.variable_scope("block3"):
                net=resnet(net,[64, 64, 128],stride=2)
                net=resnet(net,[64, 64, 128])
                net=resnet(net,[64, 64, 128])
                net=resnet(net,[64, 64, 128])
                end_points['block3'] = net
            print('block3',net.shape)
            with tf.variable_scope("block4"):
                net=resnet(net,[128, 128, 256],stride=2)
                net=resnet(net,[128, 128, 256])
                net=resnet(net,[128, 128, 256])
...
with tf.variable_scope("block0"):
            ...
            net=conv(inputs,[3,3],32,stride=2)
