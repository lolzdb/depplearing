...
def unit_neck(input,fillter,scope,shortchut,stride=1):
    with tf.variable_scope("mob_unit" + scope):
            ...
            if shortchut==True:
                output=output+input
            return output
def conv(input,num,fillter,stride=1):
...
def yolo(input,fillter,short=True,stride=1):
    ...
    if short:
        if stride==2:
            shortcut = slim.conv2d(input, fillter[1], (3, 3), activation_fn=None, data_format=data_format,stride=stride)
            shortcut = slim.batch_norm(shortcut, activation_fn=None, is_training=training, data_format=data_format)
            shortcut=tf.nn.leaky_relu(shortcut)
        output=output+shortcut
    return output

def yolod(input,fillter,short=True,stride=1):
    output = slim.conv2d(input, fillter[0], (1, 1), activation_fn=None, data_format=data_format)
    output = slim.batch_norm(output, activation_fn=None, is_training=training, data_format=data_format)
    output = tf.nn.leaky_relu(output)
    output = slim.conv2d(output, fillter[0], (3, 3), activation_fn=None, data_format=data_format,stride=stride)
    output = slim.batch_norm(output, activation_fn=None, is_training=training, data_format=data_format)
    output = tf.nn.leaky_relu(output)
    output = slim.conv2d(output, fillter[0], (1, 1), activation_fn=None, data_format=data_format)
    output = slim.batch_norm(output, activation_fn=None, is_training=training, data_format=data_format)
    output = tf.nn.leaky_relu(output)
    output = slim.conv2d(output, fillter[1], (3, 3), activation_fn=None, data_format=data_format)
    output = slim.batch_norm(output, activation_fn=None, is_training=training, data_format=data_format)
    output = tf.nn.leaky_relu(output)
    shortcut=input
    if short:
...
def ssd_net(inputs,
    ...
    with tf.variable_scope(scope, 'ssd_512_vgg', [inputs], reuse=reuse):
        ...
        with tf.variable_scope("block0"):
            net = slim.conv2d(inputs, 32, [7, 7], activation_fn=tf.nn.selu, stride=2,data_format=data_format)  # 256
        with tf.variable_scope("block1"):
            net=yolo(net,[16,32])
            net = yolo(net,[16, 32])
            net = yolo(net,[16, 32])
        with tf.variable_scope("block2"):
            net = yolo(net, [32, 64],stride=2)#128
            net = yolod(net, [32, 64])
        with tf.variable_scope("block3"):
            net = yolod(net, [64, 128], stride=2)  # 64
            net = yolod(net, [64, 128])
            net = yolod(net, [64, 128])
        end_points['block3'] = net
        with tf.variable_scope("block4"):
            net = yolod(net, [128, 256], stride=2)  # 32
            net = yolo(net, [128, 256])
        end_points['block4'] = net
        with tf.variable_scope("block5"):
            net = yolod(net, [128, 256], stride=2)  # 16
            net = yolo(net, [128, 256])
        end_points['block5'] = net
        with tf.variable_scope("block6"):
            net = yolod(net, [128, 256], stride=2)  # 8
            net = yolo(net, [128, 256])
