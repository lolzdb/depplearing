...
class SSDNet(object):
    ...
    default_params = SSDParams(
        ...
        feat_layers=['block3','block4', 'block5', 'block6', 'block7'],
...
def ssd_net(inputs,
    ...
    with tf.variable_scope(scope, 'ssd_512_vgg', [inputs], reuse=reuse):
        ...
        with tf.variable_scope("RseNet50"):
            ...
            net8 = slim.conv2d(net, 128, [1, 1], scope='conv1x1')
            net8 = slim.conv2d(net8, 256, [3, 3], scope='conv3x3', padding='VALID')
            end_points['block8'] = net8
            end_point = 'Fblock6'
            with tf.variable_scope(end_point):
                net = conv(net, 256, [1, 1])
                net = expand(net, [8, 8])
                c = conv(end_points['block6'], 256, [1, 1])
                net = net + c
                net = conv(net, 256, [1, 1])
                print(net.shape)
            ...
            end_point = 'Fblock5'
            with tf.variable_scope(end_point):
                net = expand(net, [16, 16])
                c = conv(end_points['block5'], 256, [1, 1])
                net = net + c
                net = conv(net, 256, [1, 1])
                print(net.shape)
            end_points[end_point] = net
            end_point = 'Fblock4'
            with tf.variable_scope(end_point):
                net = expand(net, [32, 32])
                c = conv(end_points['block4'], 256, [1, 1])
                net = net + c
                net = conv(net, 256, [1, 1])
                print(net.shape)
            end_points[end_point] = net
            end_point = 'Fblock3'
            with tf.variable_scope(end_point):
                net = expand(net, [64, 64])
                c = conv(end_points['block3'], 256, [1, 1])
                net = net + c
                net = conv(net, 256, [1, 1])
                print(net.shape)
            end_points[end_point] = net
