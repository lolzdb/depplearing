...
def resnet(input,fillter,short=True,stride=1):
    ...
    if short:
        if stride!=1:
            shortchut = slim.conv2d(preact, fillter[2], (1,1),activation_fn=None,data_format=data_format,stride=stride)
            output=output+shortchut
        else:
            output=output+preact
    return output

def yolo(input,fillter,short=True,stride=1):
    net=conv(input,[1,1],fillter[0],stride=1)
    net=conv(input,[3,3],fillter[1],stride=1)
    if short:
        net=input+net
    return net
