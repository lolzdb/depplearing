...
class Box:
    ...
    def iscontain(self,box):
        if box.xmin>=self.xmin and box.ymin>=self.ymin and box.xmax<=self.xmax and box.ymax<=self.ymax:
            return True
        return False
...
# d=dl
datapath='F:\\data\\test'
resultpath='F:\\data\\t1280'
ids=getjson('F:\\data\\test\\ids.json')
json_file=getjson('F:\\annotations.json')

voc,ids=crop(ids,json_file['imgs'],datapath,resultpath)
info={'imgs':voc}
writejson(resultpath+'/ids.json',ids)
